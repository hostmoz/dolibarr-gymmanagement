...
/**
 * \file        class/gymmember.class.php
 * \ingroup     gymmanagement
 * \brief       This file is a CRUD class file for GymMember (Create/Read/Update/Delete)
 */
 ...
 * Class for GymMember
 */
class GymMember extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'gymmanagement';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'gymmember';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'gymmanagement_gymmember';

	/**
	 * @var string 	If permission must be checkec with hasRight('gymmanagement', 'read') and not hasright('mymodyle', 'gymmember', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'gymmanagement';

	/**
	 * @var string 	String with name of icon for gymmember. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'gymmember@gymmanagement' if picto is file 'img/object_gymmember.png'.
	 */
	...
	//  */
	// public $table_element_line = 'gymmanagement_gymmemberline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_gymmember';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'GymMemberline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'GymMember', 'fk_element'=>'fk_gymmember'));
	...
	//  */
	// protected $childtablesoncascade = array('gymmanagement_gymmemberdet');

	// /**
	//  * @var GymMemberLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('gymmanagement', 'gymmember', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gymmanagement', 'gymmember', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gymmanagement', 'gymmember_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'gymmember/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'gymmember/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'gymmember/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'gymmember/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->gymmanagement->dir_output.'/gymmember/'.$oldref;
				$dirdest = $conf->gymmanagement->dir_output.'/gymmember/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gymmanagement->dir_output.'/gymmember/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gymmanagement','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gymmanagement','gymmanagement_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'GYMMANAGEMENT_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'GYMMANAGEMENT_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'GYMMANAGEMENT_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowGymMember")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("GymMember").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/gymmanagement/gymmember_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowGymMember");
	...
	public function getLinesArray()
	{
		...
		$objectline = new GymMemberLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_gymmember:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('GYMMANAGEMENT_MYOBJECT_ADDON')) {
			$conf->global->GYMMANAGEMENT_MYOBJECT_ADDON = 'mod_gymmember_standard';
		}

		if (getDolGlobalString('GYMMANAGEMENT_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('GYMMANAGEMENT_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('GYMMANAGEMENT_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/gymmanagement/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefGymMember $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/gymmanagement/doc/";
...
/**
 * Class GymMemberLine. You can also remove this and generate a CRUD class for lines objects.
 */
class GymMemberLine extends CommonObjectLine
{
	// To complete with content of an object GymMemberLine
	// We should have a field rowid, fk_gymmember and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'gymmember'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_gymmember'

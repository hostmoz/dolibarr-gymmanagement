...
/**
 *   	\file       gymmember_list.php
 *		\ingroup    gymmanagement
 *		\brief      List page for gymmember
 */
...
require_once __DIR__.'/class/gymmember.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('gymmanagement', 'gymmember', 'write');
	$permissiontodelete = $user->hasRight('gymmanagement', 'gymmember', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("gymmanagement")) {
	accessforbidden('Module gymmanagement not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'GymMember';
	$objectlabel = 'GymMember';
	$uploaddir = $conf->gymmanagement->dir_output;
...
$title = $langs->trans("GymMembers");
//$help_url = "EN:Module_GymMember|FR:Module_GymMember_FR|ES:Módulo_GymMember";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/gymmanagement/gymmember_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-gymmanagement page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/gymmanagement/gymmember_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendGymMemberRef";
$modelmail = "gymmember";
$objecttmp = new GymMember($db);
